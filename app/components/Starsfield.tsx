...
type Direction = "none" | "left" | "right" | "top" | "bottom"
...
function parseColor(input: string | undefined) {
    ...
    if (m) {
        ...
        const a =
            m[4] !== undefined ? Math.max(0, Math.min(1, Number(m[4]))) : 1
        return { r, g, b, a }
    }
...
// ---------- Component ----------
export default function Starsfield(props: Partial<Props>) {
    const {
        starCount = 1200,
        speed = 0.15,
        spread = 2.0,
    ...
    // Visibility State (Intersection Observer)
    const [isInView, setIsInView] = React.useState(true)

    // Cursor tracking
    const followTargetRef = React.useRef<{ x: number; y: number }>({
        x: 0,
        y: 0,
    })
    ...
    // Cache parsed colors
    const colorCacheRef = React.useRef({
    ...
    const initStars = React.useCallback(() => {
        ...
        const { focal, spread, starSize, starCount, randomColors } =
            settingsRef.current
    ...
    const drawFrame = React.useCallback(
        (
            ...
            const rgbaCache = randomColors ? null : new Map<number, string>()
            const getCachedColor = (
                ...
                if (!rgbaCache.has(key)) {
                    rgbaCache.set(
                        key,
                        `rgba(${star.r},${star.g},${star.b},${alpha})`
                    )
                }
            ...
            for (let i = 0; i < stars.length; i++) {
                ...
                s.z += speedFactor
                if (
                    (!reverseFly && s.z <= Z_NEAR) ||
                    (reverseFly && s.z >= Z_FAR)
                ) {
                    respawnStar(
                        s,
                        w,
                ...
                if (galaxyMode) {
                    ...
                    if (r > 0.0001) {
                        ...
                        const spinSpeed = Math.max(0.1, speed * 1.0)
                ...
                const twk = Math.max(
                    ...
                    Math.min(
                        ...
                        0.65 +
                            twinkle *
                                0.35 *
                                Math.sin(s.phase + twinkleSpeed * s.twinkle)
                    )
                )
                ...
                const appear =
                    smooth01(saturate(tFar)) * smooth01(saturate(tNear))
                ...
                if (m.active) {
                    ...
                    const radius = 140
                    if (mdist < radius && mdist > 0.001) {
                        const t = 1 - mdist / radius
                        const push = t * t * radius * 0.8
                        tox = (mdx / mdist) * push
                        toy = (mdy / mdist) * push
                    }
                }
